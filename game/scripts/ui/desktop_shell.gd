...
func _register_app(app_id: StringName, title: String, scene: PackedScene, position: Vector2, size: Vector2) -> void:
	if scene == null:
		push_error("DesktopShell._register_app failed: scene is null for '%s'" % app_id)
		return

	var app_instance := scene.instantiate()
	var app_control := app_instance as Control
	if app_control == null:
		push_error("DesktopShell._register_app failed: scene root is not Control for '%s'" % app_id)
	...
	if app_window == null:
		push_error("DesktopShell._register_app failed: could not instantiate AppWindow for '%s'" % app_id)
		return
...
func _on_game_event(topic: StringName, payload: Dictionary) -> void:
	if topic == &"intel_ping":
		...
		_append_feed("Case loaded: %s (seed %s)." % [payload.get("case_id", "unknown"), payload.get("seed", "n/a")])
	elif topic == &"case_station_report":
		_append_feed("Station report filed for HQ review.")
	elif topic == &"staff_status":
		_append_feed("STAFF: %s" % payload.get("message", "Status update."))
	elif topic == &"staff_analysis_ready":
		_append_feed("STAFF REPORT: %s" % payload.get("summary", "Analysis ready."))
...
func _on_case_resolved(outcome_id: StringName, summary: String) -> void:
	_append_feed("Outcome [%s]: %s" % [outcome_id, summary])
	_open_app(&"debrief")
...
func _on_station_report_ready(report: Dictionary) -> void:
	...
	_append_feed("REPORT // %s" % report.get("operational_summary", "No operational summary."))
	_append_feed("REPORT // TRUTH: %s" % report.get("ground_truth_summary", "No ground truth summary."))
